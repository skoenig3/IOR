...
for i = 1:size(eyedat,2);
    x = eyedat{i}(1,:);
    y = eyedat{i}(2,:);
    if length(x) > 1120 %500 ms cross hair + 5000 ms for image + 100 ms buffer
        notattended(i) = (length(x)-1120)/1120;
        x = x(1:1120);
        y = y(1:1120);
    else
        notattended(i) = 0;
...
imgset = ITMFile(1:end-4);

save(['C:\Users\seth.koenig\Documents\MATLAB\IOR\Eye Data\SFT\'...
   cortexfile(1:end-2) '_' cortexfile(end) '-fixation.mat'],...
    'fixationstats','initialcross','notattended','imgset')
end