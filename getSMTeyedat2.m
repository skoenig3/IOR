...
% extracts raw Shift task (SMT) eye data from a cortex files, calibrates
...
for i = 1:size(eyedat,2);
    ...
    if length(x) > 1520 %500 ms cross hair + 7000 ms for image + 100 ms buffer
        notattended(i) = (length(x)-1520)/1520;
        x = x(1:1520);
        y = y(1:1520);
    else
        notattended(i) = 0;
...
save(['C:\Users\seth.koenig\Documents\MATLAB\IOR\Eye Data\SMT\'...
