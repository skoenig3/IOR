function [eyedat,imageprez] = getVPLTeyedat(cortexfile,ITMFile,CNDFile)
itmfil=[];
h =fopen(ITMFile);
tline = 1;
while tline > 0
...
for i = 1:length(cnd);
    ...
    if cndline > 10
        ...
        if itm < 40
            img = itmfil(itm,imgind(end)+1:imgind(end)+1);
        elseif itm < 130
...
imageprez = NaN(2,ceil(length(images)/2));
for i = 1:ceil(length(images)/2)
    ind = find(images == i);
    imageprez(1,i) = ind(1);
    if length(ind) == 2;
       imageprez(2,i) = ind(2); 
